...
const header = tv({
  ...
  variants: {
    collapsed: {
      ...
      true: "mt-2 p-5 group-data-[sidebar-intent=float]/sidebar-container:mt-2 md:mx-auto md:size-9 md:items-center md:justify-center md:rounded md:p-0 md:hover:bg-(--sidebar-accent)",
    },
  },
})
...
const footer = tv({
  base: [
    ...
    " **:data-[slot=menu-trigger]:rounded",
  ...
  return (
    ...
    >
      {state !== "collapsed" && "title" in props && (
        <Header className="text-sidebar-fg/70 ring-sidebar-ring mb-1 flex shrink-0 items-center rounded px-2.5 text-xs font-medium transition-[margin,opa] duration-200 ease-linear outline-none group-data-[sidebar-collapsible=dock]/sidebar-container:-mt-8 group-data-[sidebar-collapsible=dock]/sidebar-container:opacity-0 focus-visible:ring-2 *:data-[slot=icon]:size-4 *:data-[slot=icon]:shrink-0">
          {props.title}
        </Header>
      )}
...
const sidebarItemStyles = tv({
  base: [
    "group/sidebar-item relative col-span-full cursor-pointer overflow-hidden rounded px-[calc(var(--spacing)*2.3)] py-[calc(var(--spacing)*1.3)] text-sidebar-fg/70 focus-visible:outline-hidden sm:text-sm/6",
  ...
  return (
    <main
      ...
      className={twMerge(
        ...
        "peer-data-[sidebar-intent=inset]:min-h-[calc(100svh-theme(spacing.4))] md:peer-data-[sidebar-intent=inset]:m-2 md:peer-data-[sidebar-intent=inset]:ml-0 md:peer-data-[sidebar-intent=inset]:rounded-xl md:peer-data-[sidebar-intent=inset]:shadow-xs md:peer-data-[sidebar-state=collapsed]:peer-data-[sidebar-intent=inset]:ml-2",
        className
      )}
      {...props}
    />
...
const sidebarDisclosureTrigger = tv({
  base: [
    "group relative flex w-full cursor-pointer items-center overflow-hidden rounded px-[calc(var(--spacing)*2.3)] py-[calc(var(--spacing)*1.3)] text-sidebar-fg/70 outline-hidden sm:text-sm/6",
...
const SidebarRail = ({ className, ref, ...props }: React.ComponentProps<"button">) => {
  ...
  return (
    <button
      ...
      className={twMerge(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 outline-hidden transition-all ease-linear group-data-[sidebar-side=left]/sidebar-container:-right-4 group-data-[sidebar-side=right]/sidebar-container:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-transparent sm:flex",
